...
class BendersTRandMaster(BendersMasterMILP):
    ...
    def _update_tr_bound(self, x_sol, obj_val):
        ...
        if self.trust_region_feasibility_strategy == TrustRegionStrategy.DISTANCE_CORRECTION:
            ...
            # TODO We have to amplify the gradient of every new cut with the chosen rho.
            # It is a blinded procedure, we don't have to check for possible violation or something else,
            # we only have to execute. Maybe the only cut to not amplify are the one generated by the feasibility step.
    ...
    def solve(self, nlpdata: MinlpData, prev_feasible=True, require_benders=False) -> MinlpData:
        ...
        # TODO: the gradient amplification should go here. After creating the cut we simply amplify its gradient by rho.
